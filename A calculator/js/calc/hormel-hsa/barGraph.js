...
function BarGraph(ctx) {
    ...
    var loop = function () {
        ...
        for (var i = 0; i < endArr.length; i += 1) {
            ...
            delta = (Math.round(endArr[i]) - Math.round(startArr[i])) / that.animationSteps;
            that.curArr[i] += delta;
    ...
    var draw = function (arr) {
        ...
        if (that.xAxisLabelArr.length) {
            graphAreaHeight -= that.xAxisLabelHeight;
        }
        ...
        for (i = 0; i < arr.length; i += 1) {
            ...
            if (that.maxValue) {
                ...
                ratio = arr[i] / (Math.max(largestValue, that.minMaxValue));
            }
            ...
            if (barHeight > border * 2) {
                // Create gradient
                gradient = ctx.createLinearGradient(0, 0, 0, graphAreaHeight * 1.25);
            ...
            try {
                ctx.fillText(that.valueLabelFormatter(arr[i]),
            ...
            if (that.xAxisLabelArr[i]) {
                ...
                try {
                    ...
                    var lineheight = 15;
                    var x = i * that.width / numOfBars + (that.width / numOfBars) / 2;
                    var y = (that.height - (that.xAxisLabelHeight)) + lineheight + 5;
                    var lines = that.xAxisLabelArr[i].split('\n');

                    for (var lineIx = 0; lineIx < lines.length; lineIx++)
                        ctx.fillText(lines[lineIx],
                        x, y + (lineIx * lineheight));
    ...
    this.valueLabelFormatter = function (value) {
